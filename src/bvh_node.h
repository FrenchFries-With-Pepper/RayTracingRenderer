...
int box_x_compare(const void * a, const void *b) {
	...
	if (!ah->bounding_box(0, 0, box_left) || !bh->bounding_box(0, 0, box_left))
		std::cerr << "no bounding box in bvh_node constractor\n";
...
int box_z_compare(const void *a, const void*b) {
	...
	hitable *ah = *(hitable**)a;
	hitable *bh = *(hitable**)b;
	if (!ah->bounding_box(0, 0, box_left) || !ah->bounding_box(0, 0, box_right))
		std::cerr << "no bounding box bvh_node constructor\n";
...
bvh_node::bvh_node(hitable **l, int n, float t0, float t1) {
	int axis = int(3 * get_random());//randomly choose one axis
	if (axis == 0)//x
